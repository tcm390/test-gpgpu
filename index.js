...
const batTexture = textureLoader.load(baseUrl + `./textures/bats.jpg`);
batTexture.wrapS = batTexture.wrapT = THREE.RepeatWrapping;
...
export default () => {
  ...
  const fragmentShaderPosition = `
    ...
    void main()	{
      ...
      phase = mod( ( phase + delta +
        ...
        max( velocity.y, 0.0 ) * delta * 6. ), 68.83 );
  ...
  const gltfs = [baseUrl + `./assets/Parrot.glb`, baseUrl + `./assets/Flamingo.glb`];
  const colors = [ 0xccFFFF, 0xffdeff ];
  const sizes = [ 0.02, 0.01 ];
  const selectModel = Math.floor( Math.random() * gltfs.length );
  let bat = null;

  (async () => {
    const u = gltfs[selectModel];
    bat = await new Promise((accept, reject) => {
        const {gltfLoader} = useLoaders();
        gltfLoader.load(u, accept, function onprogress() {}, reject);
        
    });
    const animations = bat.animations;
    durationAnimation = Math.round( animations[ 0 ].duration * 60 );
    const birdGeo = bat.scene.children[ 0 ].geometry;
  ...
  useFrame(({timestamp}) => {
    ...
    if (positionUniforms) {
      ...
      velocityUniforms[ 'predator' ].value.set( 0.5 * 0, - 0.5 * 0, 0 );
    }
  ...
  //       uniforms.playerPos.value.copy(player.position);
  //       app.updateMatrixWorld();
